...
@onready var crafting_controller: CraftingController = $CraftingController
...
func _ready() -> void:
	...
	if has_node("/root/GameTime"):
		get_node("/root/GameTime").connect("tick", Callable(self, "_on_tick"))

	# Initialize event system
	_last_event_time = GameTime.get_unix_time_from_system()

	# Market UI should only be opened via interaction with traders
	# No auto-opening on stage start

	_hydrate_player_character()
	call_deferred("_setup_crafting")
	if settlement:
		settlement.population_changed.connect(_on_population_changed)
		settlement.resources_changed.connect(_on_resources_changed)
		settlement.settlement_won.connect(_on_settlement_won)
		settlement.settlement_lost.connect(_on_settlement_lost)
...
func _register_with_game() -> void:
	...
	if game and game.has_method("register_economy_controllers"):
		game.register_economy_controllers(market, settlement)

func _hydrate_player_character() -> void:
	var player := get_node_or_null("PlayerSniper")
	if player == null:
	...
	if not has_node("/root/CharacterService"):
		print("⚠️ CharacterService not available; skipping character hydration")
		return

	var character_data := CharacterService.get_current_character()
	if character_data.is_empty():
		print("⚠️ No active survivor selected; using default player stats")
		return

	if player.has_method("hydrate_from_character_data"):
		player.hydrate_from_character_data(character_data)
	else:
		print("⚠️ Player does not support character hydration")

func _setup_crafting() -> void:
	if crafting_controller == null or crafting_ui == null:
		return
	var player := get_node_or_null("PlayerSniper")
	if player == null:
		player = get_tree().get_first_node_in_group("player")
	if player and player.has_node("Inventory"):
		...
		crafting_controller.set_inventory(inv)
		crafting_ui.set_inventory(inv)
	crafting_ui.set_controller(crafting_controller)
	crafting_ui.hide_ui()
	if has_node("/root/Game"):
		get_node("/root/Game").register_crafting_interface(crafting_controller, crafting_ui)
...
func _restore_persistent_state() -> void:
	...
	if typeof(save_data) != TYPE_DICTIONARY or save_data.is_empty():
		return
	if market and save_data.has("market"):
		var market_state = save_data.get("market", {})
		if typeof(market_state) == TYPE_DICTIONARY and not market_state.is_empty():
			market.hydrate_state(market_state)
	if settlement and save_data.has("settlement"):
		var settlement_state = save_data.get("settlement", {})
		if typeof(settlement_state) == TYPE_DICTIONARY and not settlement_state.is_empty():
			settlement.hydrate_state(settlement_state)
...
func _on_tick(dt: float) -> void:
	...
	if settlement:
		settlement.process_tick(dt)
	if market:
		market.process_tick(dt)
...
func _on_population_changed(new_population: int) -> void:
	print("👥 [Settlement] Population updated: %d" % new_population)

func _on_resources_changed(summary: Dictionary) -> void:
	print("📦 [Settlement] Resources -> Food:%d Ammo:%d Med:%d Morale:%.2f" % [summary.get("food", 0), summary.get("ammo", 0), summary.get("med", 0), summary.get("morale", 0.0)])

func _on_settlement_won() -> void:
	var hud = get_tree().get_first_node_in_group("hud")
	if hud and hud.has_method("show_event_notification"):
		hud.show_event_notification("Settlement thriving! Population secured.", "Settlement")
	print("🏆 Settlement victory condition reached")

func _on_settlement_lost() -> void:
	var hud = get_tree().get_first_node_in_group("hud")
	if hud and hud.has_method("show_event_notification"):
		hud.show_event_notification("Settlement collapse imminent!", "OutpostAttacked")
	print("⚠️ Settlement failure detected")
...
func _calculate_event_chance() -> float:
	...
	var time_since_last = GameTime.get_unix_time_from_system() - _last_event_time
...
func _generate_event_payload(event_type: String) -> Dictionary:
	...
	var payload := {"timestamp": GameTime.get_unix_time_from_system()}
...
func force_event(event_type: String) -> void:
	...
	if _event_pool.has(event_type):
		_trigger_event(event_type, GameTime.get_unix_time_from_system())
	else:
...
func demonstrate_godot_aegis_systems() -> void:
	...
	get_tree().create_timer(2.0).timeout.connect(func():
		# Check if scene is still valid before proceeding
	...
	get_tree().create_timer(12.0).timeout.connect(func():
		# Check if scene is still valid before proceeding
		if not is_inside_tree() or not player or not is_instance_valid(player) or not hud or not is_instance_valid(hud):
			return
...
func _exit_tree() -> void:
	if not has_node("/root/Game"):
		return
	var game = get_node("/root/Game")
	if game and game.has_method("clear_economy_controllers"):
		game.clear_economy_controllers()
