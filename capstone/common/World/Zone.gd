...
## Zone system for Safe/Hostile area designation
...
signal zone_entered(body: Node3D, zone: Zone)
signal zone_exited(body: Node3D, zone: Zone)

var entities_in_zone: Array[Node3D] = []
...
func _apply_zone_effects(body: Node3D, entering: bool) -> void:
...
func _on_npc_entered(npc: Node3D) -> void:
    """Handle NPC entering this zone"""
    if npc.has_method("enter_zone"):
        npc.enter_zone(self)

func _on_npc_exited(npc: Node3D) -> void:
...
func get_zone_info() -> Dictionary:
    ...
    return {
        ...
        "loot_multiplier": loot_multiplier,
        "noise_penalty": noise_penalty,
        "entities_in_zone": entities_in_zone.map(func(e): return e != null ? e.name : "null")
    }
