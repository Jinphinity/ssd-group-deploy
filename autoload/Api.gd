...
const STAGING_API_URL: String = "https://api-staging.yourdomain.com"  # Replace with actual staging URL
const PROD_API_URL: String = "https://api.yourdomain.com"            # Replace with actual prod URL
...
func _detect_and_set_environment() -> void:
	...
	if OS.has_feature("web"):
		...
		if current_url.contains("localhost") or current_url.contains("127.0.0.1"):
...
func _get_browser_url() -> String:
	...
	if OS.has_feature("web"):
		# Try to get URL from JavaScript
		return "localhost"  # Fallback - could be enhanced with JavaScriptBridge
	return "localhost"
